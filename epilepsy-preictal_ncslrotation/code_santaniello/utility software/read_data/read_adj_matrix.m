function A = read_adj_matrix(filename,nch)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% function: A = read_adj_matrix(filename,nch)
%
% Description: It accesses the file named "filename", reads data from it,
%              and format the data into a sequence of square, symmetric
%              matrices, each with "nch" rows (columns).
%             
...
% main loop
%--------------------------------------------------------------------------
% step 1: extract the matrices. Note that only nch*(nch+1)/2 elements are
%         stored in the file for each matrix
ndata = nch*(nch+1)/2;
fseek(fid,0,'bof');
...
% for each matrix...
for i=1:size(data,2)
    tmp = diag(data(1:nch,i));
    pointer = nch;
    for j=1:nch-1
        tmp(j,j+1:nch) = data(pointer+1:pointer+nch-j,i);
        tmp(j+1:nch,j) = tmp(j,j+1:nch)';
        pointer = pointer+nch-j;
    end
    A(i,:,:) = tmp;
    clear pointer tmp
end
