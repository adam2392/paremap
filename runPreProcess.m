% NIH034
%%- vocalization
% morlet wavelet preprocessed - global and bipolar
ALParemap_eCog_PreProcess('NIH034', 'morlet', 'vocalization', 'bipolar', 100, 50);
% ALParemap_eCog_PreProcess('NIH034', 'morlet', 'vocalization', 'global', 500, 100);
% multitaper FFT preprocessed - global and bipolar
% ALParemap_eCog_PreProcess('NIH034', 'multitaper', 'vocalization', 'global', 500, 100);
% ALParemap_eCog_PreProcess('NIH034', 'multitaper', 'vocalization', 'bipolar', 500, 100);

% NIH039
ALParemap_eCog_PreProcess('NIH039', 'morlet', 'vocalization', 'bipolar', 100, 50);
% ALParemap_eCog_PreProcess('NIH039', 'morlet', 'vocalization', 'bipolar', 500, 100);
% ALParemap_eCog_PreProcess('NIH039', 'multitaper', 'vocalization', 'global', 500, 100);
% ALParemap_eCog_PreProcess('NIH039', 'multitaper', 'vocalization', 'bipolar', 500, 100);

% NIH037
ALParemap_eCog_PreProcess('NIH037', 'morlet', 'vocalization', 'bipolar', 100, 50);
% ALParemap_eCog_PreProcess('NIH037', 'morlet', 'vocalization', 'bipolar', 500, 100);
% ALParemap_eCog_PreProcess('NIH037', 'multitaper', 'vocalization', 'global', 500, 100);
% ALParemap_eCog_PreProcess('NIH037', 'multitaper', 'vocalization', 'bipolar', 500, 100);

% plotSessionsVocalizedSpect('NIH034', 'morlet', 'bipolar');
% plotSessionsVocalizedSpect('NIH037', 'morlet', 'bipolar');
% plotSessionsVocalizedSpect('NIH039', 'morlet', 'bipolar');